...
public:
    UI(): Menu(*this) {
        ...
        auto randomOpengingButton = std::make_unique<Button>(
            *this,
        ...
        randomOpengingButton->SetCallback([] {
        ...
        chooseOpeningButton->SetCallback([] {
            std::cout << "Player chooses preferred engine opening" << std::endl;
        });

        Menu.AddButton(std::move(randomOpengingButton));
    ...
    void Render(SDL_Surface* Surface) const {
        if (HeadingFont) {
            ...
            if (textSurface) {
                ...
                destRect.y = Surface->h / 8 - destRect.h / 2;  // upper portion
